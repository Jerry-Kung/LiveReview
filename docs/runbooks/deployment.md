...
- **视频保留期与自动清理（V0.6.1 起）**：`STORAGE_VIDEO_TTL_SECONDS`（对象存储中视频的保留期，默认 259200 即 72 小时）与 `CLEANUP_INTERVAL_SECONDS`（后台扫描周期，默认 900 秒）控制云端视频的回收，`CLEANUP_ENABLED`（默认 `true`）可整体关闭清理线程。到期只删视频、不删结论：已完成的转写与复盘结论仍在库里可查可下载，但再启动识别会提示重新上传。**本地磁盘不再累积视频**：入库成功后副本即释放，任务体成功或失败都不留原始文件与切片，因此 `MEDIA_ROOT` 只需容纳「正在上传/正在处理」的那一场。若排查历史任务时发现视频已过期，属预期行为——把 `STORAGE_VIDEO_TTL_SECONDS` 调大只会影响此后新上传的任务。
...
| 要建的账号 | 怎么建 |
|---|---|
| 普通账号（`member`） | 管理员登录后，侧栏「账号管理」→ 新增 |
| 管理员账号（`admin`） | 只能在服务端跑初始化脚本（下面几节） |

界面建出来的账号**一律是普通账号**：接口的请求体里根本没有角色字段。这不是界面的限制而是接口的设计——留一个角色字段等于把提权路径开在任何人手里。**要造第二个管理员，仍然只能用脚本。**

界面上管理员那一行不显示删除按钮；直接调接口删管理员账号会返回 400「管理员账号不能删除」。删除普通账号会一并作废该账号的全部会话，它已登录的窗口下一次请求就会被弹回登录页。
