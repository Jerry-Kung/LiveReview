...
- **`backend/app/models.py`**：业务表结构声明（`UploadSession` 上传会话、`Task` 处理任务、`MediaClip` 切片记录）。任务行的 `video_expired_at` 记录视频被保留期清理的时刻（V0.6.1）。
...
- **`backend/app/tasks/`**：后台任务执行器（进程内线程池）、任务状态流转、启动恢复与定期清理（`cleanup.py`：无主本地产物清扫、过期视频回收），详见下节。
...
- **生命周期（V0.6.1 起本地不留视频）**：合并产物上传对象存储成功后转存为 `originals/` 下的副本，供探测与切分使用；**入库成功后该副本即释放**，任务体（`_process`）另有一层 `finally` 兜底，成功与失败都不留本地视频——失败不再保留副本用于容器内复现，复现改为按对象键从对象存储取回同一路径。切片产物在上传对象存储成功后立即删除。分片与临时文件在对象就位后清理，入库失败只清合并临时文件、保留分片，使用户无需重传整个 1～2GB 原文件。启动时按「库中是否还有对应任务/会话」清扫无主的残留文件（见「后台任务」）。
...
- **定期清理（V0.6.1）**：进程内守护线程按 `CLEANUP_INTERVAL_SECONDS`（默认 15 分钟）扫描并删除到期视频（见「对象存储 · 保留期」），`CLEANUP_ENABLED=false` 可关闭；单轮失败只记日志、不终止线程。启动时另做一次无主本地产物清扫。两者都不参与任务排队，也不占用任务线程池的并发额度。
...
- **保留期（V0.6.1 起）**：桶内的原始视频与切片只保留 `STORAGE_VIDEO_TTL_SECONDS`（默认 72 小时），到期由应用层定时清理删除——不依赖存储侧的生命周期规则。清理**只删视频、不删结论**：转写与复盘结论已落库，过期任务仍可查看与下载报告，只是不能再启动或重试识别。任务行以 `video_expired_at` 标记过期，并清空 `object_key`；重传由用户重新上传（新建任务），不做断点续跑。
- **为什么是应用层**：本版的清理与「哪些任务还在跑」这类业务状态强相关（在途任务必须跳过），放在应用里判断比在桶上配规则更准确。
...
- **角色（V0.5.2 起）**：`users.role` 有两个取值——`admin` 由初始化脚本创建，`member` 由界面上的账号管理创建。判定只有一处：`require_admin`（写成 `require_user` 的子依赖，因此同源校验与会话查库在同一个请求里仍只做一遍）决定「能不能管账号」。**这不是通用权限体系**：上传、处理、复盘、下载对所有登录账号一视同仁，也没有用户隔离——所有账号看到同一套任务数据。
- **越权与未登录是两种拒绝**：未登录 401，已登录但非管理员 403。前端把 401 统一当成「会话失效」处理，用 403 才不会被误当成掉线而弹回登录页。
- **建号**：两条路径——管理员账号走 `python -m app.auth.init_admin`（交互式；`--list` 查看、`--force` 非交互重设），普通账号可在界面的「账号管理」里增删。**界面只能建普通账号**，接口的请求体里没有角色字段；提权仍然只能靠脚本。本版仍没有注册、改口令界面，也没有用户隔离。
